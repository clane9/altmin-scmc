function eval_SCMC_methods_MS_stacked_030818(prefix,form,subset,n,D,rhos,seed,initMode,...
...
[X, groupsTrue] = load_Hopkins155_stacked(datapath, subset);
if D < size(X,1)
  X = X(1:D, :); % Select first frames if needed.
else
  D = size(X,1);
end
...
% Normalize.
% NOTE: not so well justified, since subspaces are affine. But ok if viewing
% each as linear subspace in R^4.
...
fname = sprintf('%s_%s_%s_n%d_D%d_seed%d_init%s_lamb%.1e_gamma%.1f_eta1%.1e_Ud%d_Gamma%.1e_alpha%.1e_tau1%d_lambIncr%.2f_job%05d.mat', ...
    prefix, form, subset, n, D, seed, initMode, lambda, gamma, eta1, Ud, Gamma, alpha, ...
    tau1, lambdaIncr, jobIdx);

save(fname, 'form', 'subset', 'n', 'D', 'subclasses', 'rhos', 'seed', 'initMode', 'lambda', 'gamma', 'eta1', 'Ud', 'Gamma', 'alpha', ...
