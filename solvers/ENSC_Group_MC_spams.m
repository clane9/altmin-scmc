classdef ENSC_Group_MC_spams < ENSC_MC_spams
% ENSC_Group_MC_spams   Solver for alternating elastic-net subspace clustering (with
%   projection), and group-wise completion. Uses SPAMS instead of admm so compute
%   self-expression C.
%
%   solver = ENSC_Group_MC_spams(X, Omega, n, lambda, gamma)

  methods

    function self = ENSC_Group_MC_spams(X, Omega, n, lambda, gamma)
    % ENSC_Group_MC_spams   Solver for alternating elastic-net subspace clustering (with
    ...
    %
    %   Returns:
    %     self: ENSC_Group_MC_spams solver instance.
    self = self@ENSC_MC_spams(X, Omega, n, lambda, gamma, 0);
    end
